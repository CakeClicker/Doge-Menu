...
void Hooking::FindPatterns()
{
	...
	auto p_frameCount = pattern("F3 0F 10 0D ? ? ? ? 44 89 6B 08");
	...
	//48 8B C4 48 89 58 08 48 89 68 10 48 89 70 18 48 89 78 20 41 56 48 81 EC ? ? ? ? 45 8B F0 41 8B F9 //TRIGGER SCRIPT EVENT
	...
	Log::Msg("Getting Player Pointer...");
	c_location = p_playerPtr.count(1).get(0).get<char>(0);
	c_location == nullptr ? FailPatterns("player pointer") : m_playerPtr = reinterpret_cast<uint64_t>(c_location) + *reinterpret_cast<int*>(reinterpret_cast<uint64_t>(c_location) + 3) + 7;
